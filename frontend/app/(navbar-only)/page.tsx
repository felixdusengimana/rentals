"use client"
// PropertiesGrid component with your data fetching
...
const fetchProperties = async () => {
  const { data } = await axios.get<IProperty[]>(`${process.env.NEXT_PUBLIC_BASE_URL}/properties`);
  return data;
};
...
// Header Component
const TravelSearchHeader = () => {
  return (
    <div className="w-full bg-white shadow-sm">
      {/* Search Bar */}
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="flex items-center justify-between p-2 bg-white rounded-full border shadow-md">
          <div className="flex-1 px-4 py-2 border-r">
            <p className="text-xs font-medium text-gray-600">Where</p>
          ...
          <div className="flex-1 px-4 py-2">
            <p className="text-xs font-medium text-gray-600">Who</p>
            <p className="text-sm">Add guests</p>
          </div>
          
          <button className="flex items-center justify-center h-12 w-12 bg-blue-600 hover:bg-blue-700 text-white rounded-full">
      ...
      <div className="max-w-7xl mx-auto px-4 pb-4">
        <div className="flex items-center space-x-8 overflow-x-auto scrollbar-hide pb-4">
          <CategoryButton icon="🏝️" label="Beach" active />
          <CategoryButton icon="⛰️" label="Mountains" />
          <CategoryButton icon="❄️" label="Arctic" />
          <CategoryButton icon="🏡" label="Cabins" />
          <CategoryButton icon="✨" label="Luxe" />
          <CategoryButton icon="🏝️" label="Islands" />
          <CategoryButton icon="🚜" label="Farms" />
          <CategoryButton icon="🚐" label="Campers" />
          <CategoryButton icon="⛺" label="Camping" />
          <CategoryButton icon="🌴" label="Tropical" />
        </div>
...
const CategoryButton = ({ icon, label, active = false }) => {
  return (
    <button className={`flex flex-col items-center justify-center space-y-1 min-w-16 py-2 opacity-80 hover:opacity-100 ${active ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500'}`}>
...
// Main App Page - Putting it all together
import { NextPage } from 'next';
import { QueryClient, QueryClientProvider } from '@tanstack/react-query';
import { BiChevronLeft, BiChevronRight, BiHeart, BiMap } from 'react-icons/bi';
import HomePropertyCard from '@/src/components/HomePropertyCard';
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="border-t mt-8 pt-8 flex flex-col md:flex-row md:justify-between">
          <div className="flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-4">
            <span className="text-sm">© 2025 Your Company, Inc.</span>
