...
export default function PropertiesList() {
    ...
    const { data: properties, isPending, isFetching, isLoading } = useQuery({
        queryKey: ["properties", user?.id, id],
        queryFn: async () => {
          const res = await api.get<IPaginatedResponse<IProperty>>(`${process.env.NEXT_PUBLIC_BASE_URL}/properties/by-user/${user?.id}?parentId=${id}`);
          return res.data;
        },
        retry: 3,
        enabled: !!user,
        ...
        <div className='flex justify-between items-center mb-4'>
            <div className=''>
                ...
                <p className='text-gray-500 text-sm'>
                    List of all rooms for <span className='text-blue-500'>{property?.title}</span>
                </p>
            </div>
            <AddProperty parent={property}  trigger={<Button>Add Room</Button>}/>
        </div>
