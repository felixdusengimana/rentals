...
export default function BookingsList() {
    ...
    const { data: properties, isPending, isFetching, isLoading } = useQuery({
        queryKey: ["bookings"],
        queryFn: async () => {
          const res = await api.get<IPaginatedResponse<IBooking>>(`${process.env.NEXT_PUBLIC_BASE_URL}/bookings/my/${user?.id   }/?userType=HOST`);
          return res.data;
        },
        retry: 3,
        enabled: !!user,
            ...
            columnHelper.accessor('property.id', {
                ...
                cell: (info) => <p className='flex items-center'>
                    <Dropdown trigger={<BiDotsHorizontal />}>
                        ...
                        <ViewProperty
                        id={info.row.original?.property?.id!}
        ...
        <div className='flex justify-between items-center mb-4'>
            <div className=''>
                ...
                <p className='text-gray-500 text-sm'>
                    List of all booking for your properties
                </p>
            </div>
        </div>
